...
def do_deploy(archive_path):
    ...
    run('tar -xzf /tmp/{0} -C /data/web_static/releases/{0}/'.format(file_name))
    run('rm /tmp/{}'.format(file_name))
    run('mv /data/web_static/releases/{0}/web_static/* /data/web_static/releases/{0}/'.format(file_name))
